...
struct Can2Feedback can2Feedback;
...
static void canGpioNvicInit(CAN_TypeDef *can_x)
{
	...
	if (can_x == CAN1)
	{
  #if defined USE_MAIN_CTRL_2021_PJ||defined USE_MAIN_CTRL_2021_B	\
...
void CAN1_RX0_IRQHandler(void)
{
	if (CAN_GetITStatus(CAN1, CAN_IT_FMP0) != RESET)
	{
		...
		case 160:
		{
			...
			memcpy(&powerHeatData,Can1RxMsg.Data,sizeof(powerHeatData));
			break;
			can1Feedback.Jgmt_OutSpd = powerHeatData.bulletSpeed/10.0;
		}
		case 140:
		{
			can1Feedback.jgmtOffline = 0;
			can1Feedback.jgmtOfflineCheck = 0;
			memcpy(&jgmtInfoMsg,Can1RxMsg.Data,sizeof(jgmtInfoMsg));
			break;
		}
		case 110:
		{
			...
			memcpy(&powerInfoMsg,Can1RxMsg.Data,sizeof(powerInfoMsg));
			break;
		}
		case 130:
		{
			can1Feedback.SuperPowerOfflineCheck = 0;
			can1Feedback.SuperPowerOffline = 0;
			memcpy(&powerRecvMsg,Can1RxMsg.Data,sizeof(powerRecvMsg));
			break;
		}
		case 0x103:
		{
			can1Feedback.GYdistance[0] = (float)(((Can1RxMsg.Data[0] << 8) | (Can1RxMsg.Data[1])) / 200.0f);
			can1Feedback.GYdistance[1] = (float)(((Can1RxMsg.Data[2] << 8) | (Can1RxMsg.Data[3])) / 200.0f);
			can1Feedback.GYdistance[2] = (float)(((Can1RxMsg.Data[4] << 8) | (Can1RxMsg.Data[5])) / 200.0f);
			break;
		}
		default:
			break;
...
void CAN2_RX0_IRQHandler(void)
{
	if (CAN_GetITStatus(CAN2, CAN_IT_FMP0) != RESET)
	{
		...
		case 0x101:
		{
			can2Feedback.frictionShootNum = (int16_t)((Can2RxMsg.Data[0] << 8) | (Can2RxMsg.Data[1]));

			can2Feedback.Snail_A_FeedbackSpd_Now = (int16_t)((Can2RxMsg.Data[2] << 8) | (Can2RxMsg.Data[3]));
			can2Feedback.Snail_B_FeedbackSpd_Now = (int16_t)((Can2RxMsg.Data[4] << 8) | (Can2RxMsg.Data[5]));

			can2Feedback.AS5048_offline = Can2RxMsg.Data[6];

			can2Feedback.ready_to_fire = (int16_t)(Can2RxMsg.Data[7]);
			can2Feedback.frictionOfflineCheck++; //摩擦轮离线检查
			can2Feedback.frictionOffline = 0;
			break;
		}
		default:
			break;
