...
{
	if (jgmtInfoMsg.coolingLimit == -1 || unLimitedFired == 1)
	{
...
void FireCtrl::jgmtHeatCalc()
{
	if (!can1Feedback.jgmtOffline)
		jgmtHeat = jgmtInfoMsg.coolingLimit;
	else
...
void FireCtrl::run()
{
	...
	if (powerHeatData.coolingHeat > jgmtInfoMsg.coolingLimit || fireCtrl.forceOpenLoop)
	{
...
void heatMult()
{
	jgmtHeatNow = powerHeatData.coolingHeat;
...
void FireCtrl::getFireLimit()
{
	if (!can1Feedback.jgmtOffline)
	{
		fireCtrl.jgmtCoolDown = jgmtInfoMsg.coolingRate;
		fireCtrl.jgmtBulletSpeedMax = jgmtInfoMsg.speedLimit;
		fireCtrl.jgmtHeatMax = jgmtInfoMsg.coolingLimit;
	}
	else
	{
