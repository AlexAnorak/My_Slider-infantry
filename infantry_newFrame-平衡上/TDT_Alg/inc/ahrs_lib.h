...
/**
  ...
  * @author         luopin
  * @param[in]      quat 需要初始化的四元数数组
  * @param[in]      accel 用于初始化的加速度计,(x,y,z)不为空 单位 m/s2
  * @param[in]      mag 用于初始化的磁力计计,(x,y,z)不为空 单位 uT
  * @retval         返回空
  */
...
/**
  ...
  * @author         luopin
  * @param[in]      quat 需要更新的四元数数组
  * @param[in]      timing_time 更新定时时间，固定定时调用，例如1000Hz，传入的数据为0.001f
  * @param[in]      gyro 用于更新的陀螺仪数据,数组顺序(x,y,z) 单位 rad
  * @param[in]      accel 用于初始化的加速度数据,数组顺序(x,y,z) 单位 m/s2
  * @param[in]      mag 用于初始化的磁力计数据,数组顺序(x,y,z) 单位 uT
  * @retval         返回空
  */
...
/**
  ...
  * @author         luopin
  * @param[in]      quat 四元数数组，不为NULL
  * @param[in]      yaw 返回的偏航角yaw 单位 rad
  * @param[in]      pitch 返回的俯仰角pitch  单位 rad
  * @param[in]      roll 返回的横滚角roll 单位 rad
  */
