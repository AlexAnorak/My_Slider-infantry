...
void LED_Error(u8 isA_Offline, u8 isB_Offline)
{
	static int16_t OfflineTimer = 0;
	static int16_t Step = 0;
	if(isA_Offline == 0 && isB_Offline == 0)
	{
		return;
	}
	if(Step == 0)
	{
		if(isA_Offline)CH3_LED_ON;
		if(isB_Offline)CH4_LED_ON;
	...
	if(Step == 1)
	{
		if(isA_Offline)CH3_LED_OFF;
		if(isB_Offline)CH4_LED_OFF;
