...
/**
  ...
  */
Spi::Spi(SPI_TypeDef* spix,uint16_t baud)
{
	this->spix=spix;
	this->baud =baud;
}
...
void Spi::init(void)
{
	...
	else if(spix == SPI3 && initFlag[SPI::spi3] == false)
	{
		...
		GPIO_PinAFConfig(GPIOB,GPIO_PinSource3,GPIO_AF_SPI3); //PA5复用为 SPI1  CLK//PB3
		GPIO_PinAFConfig(GPIOB,GPIO_PinSource4,GPIO_AF_SPI3); //PA6复用为 SPI1  MISO//PB4
		GPIO_PinAFConfig(GPIOB,GPIO_PinSource5,GPIO_AF_SPI3); //PA7复用为 SPI1  MOSI//PB5
