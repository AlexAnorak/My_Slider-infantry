...
static void canGpioNvicInit(CAN_TypeDef *can_x)
{
	...
	if (can_x == CAN1)
	{
#if defined USE_MAIN_CTRL_2021_PJ || defined USE_MAIN_CTRL_2021_B || defined USE_MAIN_CTRL_2021_A || defined USE_MAIN_CTRL_2020
...
void CAN1_RX0_IRQHandler(void)
{
	if (CAN_GetITStatus(CAN1, CAN_IT_FMP0) != RESET)
	{
		...
		case 0x207:
		{
			int16_t yawEncoder= (int16_t)((Can1RxMsg.Data[0] << 8) | (Can1RxMsg.Data[1])); //机械角度
			
			/*计算校准后的机械角度*/
			int16_t yawEncoderCalibration = yawEncoder - chassis.yawZeroAngle;
			if (yawEncoderCalibration > 4096)
			{
				yawEncoderCalibration -= 8192;
			}
			else if (yawEncoderCalibration < -4096)
			{
				yawEncoderCalibration += 8192;
			}
			chassis.yawAngle = yawEncoderCalibration;
		}
			break;
		case 0x150:
			memcpy(RC.rcDataFromCan, Can1RxMsg.Data, 8);
			RC.updated = 1;
			RC.updateTime = getSysTimeUs();
			break;
		case 0x170:
			memcpy(stateCtrl.stateStructFromCan, Can1RxMsg.Data, 8);
			break;
		case 0x130:
		...
		{
			u8 firstRecv = 0;
			if(powerRecvMsg.capacitance_percentage == 0)
			{
				firstRecv = 1;
			}
			memcpy((u8*)(&powerRecvMsg), Can1RxMsg.Data, sizeof(powerRecvMsg));

			if(powerRecvMsg.capacitance_percentage < 50 && firstRecv == 1)
			{
