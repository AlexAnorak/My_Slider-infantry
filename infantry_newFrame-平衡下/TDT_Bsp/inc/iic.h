...
public:
    /**
     * @brief Hardiic构造器
     * @param iicSclPort SCL端口
     * @param iicSclPin SCL引脚
     * @param iicSdaPin SDA引脚
     * @param IIC IIC端口
     * @param iicFrequency IIC通信频率 单位：Hz  默认400kHz，范围400kHz-1Hz
     * @param iicSdaPort SDA端口 默认为0，即与SCL相同
     * @param ulTimeOut_Time 硬件IIC等待总线超时时间 默认为0，即系统时钟频率/10000
     */
...
private:
    ...
    ///@brief iic通信频率 初始输入频率经过计算后会换算为延时时间储存在此变量中
    ///@warning 当IIC通信频率过高时某些外设可能不支持
    uint32_t iicFrequency; 
...
public:
    /**
     * @brief Softiic构造器
     * @param[in] iicSclPort SCL端口
     * @param[in] iicSclPin SCL引脚
     * @param[in] iicSdaPin SDA引脚
     * @param[in] IIC IIC端口（占位变量，用于硬件iic和软件iic随意切换）
     * @param[in] iicFrequency IIC通信频率 单位：Hz  默认1MHz，范围1.2MHz-10kHz
     * @param[in] iicSdaPort DA端口 默认为0，即与SCL相同
     */
