/*****************************************************************************
File name: TDT_Device\src\mpu6050.h
Description: 陀螺仪
Author: 祖传
...
#define MPU6050_ADDRESS 0x68 // 0x68
...
#define _USE_HARDIIC 0

#if _USE_HARDIIC
class Mpu6050 : public Hardiic, public ImuCalc
#else
class Mpu6050 : public Softiic, public ImuCalc
#endif
{
private:
    void Mpu6050_Read_Acc_Data(void);  //读取加速度
    void Mpu6050_Read_Gyro_Data(void); //读取角速度
    void Mpu6050_Read(void);           //读取加速度和角速度数据

    ImuCalc *imuView;

public:
    Mpu6050(GPIO_TypeDef *iicPort, uint16_t iicSclPin, uint16_t iicSdaPin = 0, GPIO_TypeDef *iicSdaPort = 0, I2C_TypeDef *IIC = I2C1);

    void init(uint16_t sample_rate, uint16_t lpf);
    virtual void init() override { init(1000, 42); };

    virtual void gyroAccUpdate() override { Mpu6050_Read(); }
};
