...
float Pid::Calculate(double _setValue, double _fbValue, int planIndex, float *_result, float _T)
{
	...
	else if (paramPtr[planIndex].integralMethod == 1)
	{
		...
		{
			if (ABS(lastError) > paramPtr[planIndex].integralThreshold)
				integralError = 0;
			integralError += 0.5f * (error + lastError) * _T;
		}
		else
		{
			if (paramPtr[planIndex].kiAmpForIntegralSep > 0)
				integralError += paramPtr[planIndex].kiAmpForIntegralSep * 0.5f * (error + lastError) * _T;
			else
				integralError += 0.5f * (error + lastError) * _T;
		}
	}
